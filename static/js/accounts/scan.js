...
function unifiedLoginStart() {
    ...
    if (_loginMethod === 'phone') {
        ...
        fetch('/api/platform/'+_loginPlatform+'/login/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:aid,method:'phone',phone:phone,site_url:siteUrl})}).then(safeJson).then(function(data){
    ...
    fetch('/api/platform/'+_loginPlatform+'/login/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:aid,username:username,password:password,site_url:siteUrl})}).then(safeJson).then(function(data){
        if(data.logged_in&&data.cookies){setStepDone(1,'登录页已打开');setStepDone(2,'已填写账号密码');setStepDone(3,'无需验证码');setStepDone(4,'无需验证码');setStepDone(5,'Cookie已获取');setStepActive(6,'Cookie校验中...');setFieldValue('cfg_cookie',data.cookies);_loginResultCookie=data.cookies;showLoginStatus('info','⏳ Cookie已获取，正在校验...');doVerifyAndEnableConfirm(data.cookies,aid);}
        else if(data.needs_captcha&&data.image){setStepDone(1,'登录页已打开');setStepDone(2,'已填写账号密码');setStepDone(3,'检测到验证码');showCaptchaInput(data.image,data.captcha_type==='checkbox'||data.captcha_type==='text',data.captcha_image_url);var msg='🔒 需要验证码';if(data.captcha_type==='text')msg='🔢 请输入图片中的验证码';else if(data.captcha_type==='checkbox')msg='☑️ 请处理复选框验证码';else if(data.captcha_type==='recaptcha')msg='🔒 Google reCAPTCHA';showLoginStatus('warning',msg);}
        else if(data.success&&data.needs_captcha){setStepDone(1,'登录页已打开');setStepDone(2,'已填写账号密码');setStepDone(3,'检测到验证码');showCaptchaInput(data.image||'',true,data.captcha_image_url);showLoginStatus('warning','🔒 需要验证码处理');}
        else{setStepDone(1,'登录页已打开');setStepDone(2,'已填写账号密码');setStepDone(3,'无需验证码');setStepDone(4,'无需验证码');setStepFailed(5,data.error||'登录失败');showLoginStatus('error','❌ '+(data.error||'登录失败'));btn.disabled=false;btn.textContent='🚀 开始浏览器登录';_isLoginRunning=false;}
...
function unifiedLoginCaptcha() {
    ...
    fetch('/api/platform/'+_loginPlatform+'/login/captcha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:aid})}).then(safeJson).then(function(data){
        ...
        else if(data.needs_captcha&&data.image){showCaptchaInput(data.image,data.captcha_type==='checkbox',data.captcha_image_url);showLoginStatus('warning','🔒 请输入验证码');btn.disabled=false;btn.textContent='✅ 点验证码并登录';}
...
function unifiedSubmitCaptcha() {
    ...
    var aid=parseInt(document.getElementById('editAccountId').value)||0;setStepActive(4,'提交验证码核验中...');
    fetch('/api/platform/'+_loginPlatform+'/login/submit_captcha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:aid,captcha_code:code})}).then(safeJson).then(function(data){
        if(data.logged_in&&data.cookies){setStepDone(4,'验证码通过');setStepDone(5,'Cookie已获取');setStepActive(6,'Cookie校验中...');setFieldValue('cfg_cookie',data.cookies);_loginResultCookie=data.cookies;showLoginStatus('info','⏳ Cookie已获取，正在校验...');hideCaptchaInput();doVerifyAndEnableConfirm(data.cookies,aid);}
        else if(data.needs_captcha&&data.image){setStepActive(3,'需要新验证码');showCaptchaInput(data.image,true,data.captcha_image_url);showCaptchaFeedback((data.error||'验证码错误')+'，请重新输入',true);showLoginStatus('warning','🔒 '+(data.error||'验证码错误，请重试'));btn.disabled=false;btn.textContent='✅ 提交验证码';}
        else if(data.captcha_verified&&!data.logged_in){setStepDone(4,'验证码核验通过 ✓');setStepActive(5,'获取Cookie中...');showCaptchaFeedback('✅ 验证码通过，正在登录...',false);showLoginStatus('info','⏳ 验证码通过，正在自动登录...');btn.textContent='⏳ 登录中...';setTimeout(function(){unifiedPollLoginResult(aid)},2000);}
        else{setStepFailed(4,data.error||'验证码处理失败');showCaptchaFeedback(data.error||'处理失败',true);showLoginStatus('error','❌ '+(data.error||'验证码处理失败'));btn.disabled=false;btn.textContent='✅ 提交验证码';}
    }).catch(function(e){setStepFailed(4,'网络错误: '+e.message);showCaptchaFeedback('网络错误',true);showLoginStatus('error','❌ 网络错误: '+e.message);btn.disabled=false;btn.textContent='✅ 提交验证码';});
}
function unifiedPollLoginResult(aid) {
    fetch('/api/platform/'+_loginPlatform+'/login/poll_result',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:aid})}).then(safeJson).then(function(data){
        if(data.logged_in&&data.cookies){setStepDone(4,'验证码通过');setStepDone(5,'Cookie已获取');setStepActive(6,'Cookie校验中...');setFieldValue('cfg_cookie',data.cookies);_loginResultCookie=data.cookies;showLoginStatus('info','⏳ Cookie已获取，正在校验...');hideCaptchaInput();doVerifyAndEnableConfirm(data.cookies,aid);}
        else if(data.needs_captcha&&data.image){showCaptchaInput(data.image,true,data.captcha_image_url);showCaptchaFeedback('需要新验证码',true);showLoginStatus('warning','🔒 需要新验证码');document.getElementById('btnSubmitCaptcha').disabled=false;document.getElementById('btnSubmitCaptcha').textContent='✅ 提交验证码';}
...
function unifiedRefreshCaptcha() {
    ...
    fetch('/api/platform/'+_loginPlatform+'/login/refresh_captcha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:parseInt(document.getElementById('editAccountId').value)||0})}).then(safeJson).then(function(data){
        if(data.success&&(data.captcha_image_url||data.image)){var src=data.image?'data:image/png;base64,'+data.image:(data.captcha_image_url||'');document.getElementById('captchaImage').src=src;document.getElementById('loginScreenshotImg').src=src;document.getElementById('captchaCodeInput').value='';showCaptchaFeedback('新验证码已获取',false);}
...
function unifiedAutoCaptcha() {
    ...
    fetch('/api/platform/'+_loginPlatform+'/login/auto_captcha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:parseInt(document.getElementById('editAccountId').value)||0})}).then(safeJson).then(function(data){
...
function doVerifyAndEnableConfirm(cookie, aid) {
    ...
    fetch('/api/accounts/test-connection', {
        ...
        if (data.logged_in === true) {
            setStepDone(6, '✅ ' + (data.status || 'Cookie校验通过'));
            setStepDone(7, '🎉 登录成功');
            ...
            var cancelBtn = document.querySelector('#addFormBox form button[onclick*="unifiedClose"]');
            if (!cancelBtn) {
                var btnGroup = document.querySelector('#addFormBox .btn-group') || saveBtn.parentNode;
                cancelBtn = document.createElement('button');
                cancelBtn.type = 'button';
                cancelBtn.onclick = unifiedCancelLogin;
                cancelBtn.className = 'btn';
                cancelBtn.id = 'btnCancelLogin';
                cancelBtn.style.cssText = 'background:#fee2e2;color:#dc2626;font-weight:bold;font-size:15px;border:1px solid #fecaca;border-radius:6px;padding:8px 16px;cursor:pointer;margin-left:8px;';
            ...
            showLoginStatus('success', statusMsg + '。请点击「确认添加」保存账号，或「取消」放弃');
            document.getElementById('btnStartLogin').disabled = true;
            document.getElementById('btnStartLogin').textContent = '✅ 已登录';
        } else {
            setStepFailed(6, (data.status || 'Cookie校验失败'));
            setStepActive(5, 'Cookie获取失败，重试中...');
            ...
            retryBtn.textContent = '🔄 重新获取Cookie';
            _isLoginRunning = false;
        }
    }).catch(function(e){
        setStepFailed(6, '校验请求失败: ' + e.message);
        showLoginStatus('error', '❌ Cookie校验请求失败: ' + e.message);
        _isLoginRunning = false;
    });
}
// ❌ 取消登录 — 关闭浏览器、清理状态、关闭弹窗
function unifiedCancelLogin() {
    if (_qrSessionId) {
    ...
    if (_isLoginRunning || _loginSession || _loginReadyToSave) {
...
function unifiedRefreshScreenshot() {
    ...
    if(_loginMethod==='qrcode'&&_qrSessionId){fetch('/api/login/qrcode/'+platform+'/poll/'+_qrSessionId).then(safeJson).then(function(data){if(data.success&&data.image)document.getElementById('loginScreenshotImg').src='data:image/png;base64,'+data.image;if(data.status==='logged_in'&&data.cookies){clearInterval(_qrPollTimer);_qrPollTimer=null;setFieldValue('cfg_cookie',data.cookies);showLoginStatus('success','✅ 登录成功！');}else{showLoginStatus('info','🔄 '+(data.message||'截图已刷新'));}}).catch(function(){});return;}
    fetch('/api/platform/'+platform+'/login/screenshot').then(safeJson).then(function(data){if(data.success&&data.image)document.getElementById('loginScreenshotImg').src='data:image/png;base64,'+data.image}).catch(function(){});
}
...
function startQrCodeLogin() {
    ...
    fetch('/api/login/qrcode/'+_loginPlatform+'/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(reqBody)}).then(safeJson).then(function(data){
...
function batchRefreshAccounts() {
    ...
    fetch('/api/accounts/batch/refresh',{method:'POST'}).then(safeJson).then(function(data){
