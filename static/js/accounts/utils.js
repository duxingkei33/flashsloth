...
function toggleKeepAlive(aid, checkbox) { fetch('/api/accounts/'+aid+'/keep_alive',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({keep_alive:checkbox.checked})}).then(function(r){return r.json()}).then(function(d){if(!d.success)checkbox.checked=!checkbox.checked}).catch(function(){checkbox.checked=!checkbox.checked}); }
...
function refreshAllStatuses(forceRefresh) {
    document.querySelectorAll('[id^="statusBadge_"]').forEach(function(span) {
        ...
        fetch(url).then(function(r){return r.json()}).then(function(data){if(data.success||data.logged_in!==undefined)renderDeepStatus(id,data)}).catch(function(){span.innerHTML='<span class="badge badge-secondary">⏳ 未知</span>'});
    });
}
...
function checkStatus(aid) {
    ...
    fetch('/api/accounts/'+aid+'/status?refresh=1').then(function(r){return r.json()}).then(function(data){
...
function testSavedAccount(aid) {
    ...
    fetch('/api/accounts/test/'+aid,{method:'POST'}).then(function(r){return r.json()}).then(function(data){
        var h='<div style="margin-bottom:12px;"><p><strong>平台：</strong>'+(data.platform||'—')+'</p><p><strong>账号：</strong>'+escapeHtml(data.account_name||'')+'</p>';
        if(data.logged_in&&data.username){h+='<p><strong>用户名：</strong><span style="color:#22c55e;font-weight:600;font-size:16px;">'+escapeHtml(data.username)+'</span></p>';if(data.points)h+='<p><strong>'+(data.points_label||'积分')+'：</strong><span style="font-size:15px;color:#4361ee;">'+data.points+'</span></p>';if(data.level)h+='<p><strong>等级：</strong>'+escapeHtml(data.level)+'</p>';}
...
function testConnectionAdd() {
    ...
    fetch('/api/accounts/test-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({platform:p,config:f})}).then(function(r){return r.json()}).then(function(d){
...
function showCaptchaInput(imageBase64,needAmobbsHack){document.getElementById('loginScreenshotArea').style.display='block';document.getElementById('loginScreenshotImg').src='data:image/png;base64,'+imageBase64;document.getElementById('captchaInputArea').style.display='block';document.getElementById('captchaImage').src='data:image/png;base64,'+imageBase64;document.getElementById('captchaCodeInput').value='';document.getElementById('captchaCodeInput').focus();document.getElementById('captchaFeedback').style.display='none';document.getElementById('btnCaptchaLogin').style.display='none';document.getElementById('btnRefreshScreenshot').style.display='inline-block';var a=_loginPlatform==='amobbs'||needAmobbsHack;if(a){var note=document.createElement('div');note.id='amobbsCaptchaNote';note.style.cssText='margin-top:6px;padding:6px 10px;background:#e3f2fd;border:1px solid #90caf9;border-radius:6px;font-size:11px;color:#1565c0;';note.innerHTML='💡 Amobbs 提示：输入验证码后点击边框附近区域触发 ✓ 核验，通过后自动登录';var ex=document.getElementById('amobbsCaptchaNote');if(!ex)document.getElementById('captchaInputArea').appendChild(note);}setStepActive(4,'等待输入验证码'+(a?'（Amobbs 需点击边框核验）':''));}
function hideCaptchaInput(){document.getElementById('captchaInputArea').style.display='none';document.getElementById('captchaImage').src='';document.getElementById('captchaCodeInput').value='';var note=document.getElementById('amobbsCaptchaNote');if(note)note.remove();}
