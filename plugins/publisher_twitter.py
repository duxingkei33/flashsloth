...
class TwitterPublisher(Publisher):
    ...
    def test_connection(self) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Twitter API 异常: {e}", "status": "API 异常"}
    ...
    def upload_image(self, local_path: str) -> dict:
        ...
        返回: {"success": True, "url": "https://t.co/...", "media_id": "12345"}
        ...
        try:
            ...
            if size > self.PLATFORM_LIMITS["max_image_size"]:
                return {
                    "success": False, "url": "", "media_id": "",
                    "error": f"图片过大 ({size/1024/1024:.1f}MB > {self.PLATFORM_LIMITS['max_image_size']/1024/1024:.0f}MB)"
                }

            # 上传到 Twitter
            self.logger.info(f"⏫ 上传图片: {local_path}")
    ...
    def _upload_article_images(self, article: Article) -> tuple[str, list[str]]:
        ...
        返回:
            - body: 更新过图片 URL 的正文（Twitter CDN URL）
        ...
        for path in image_paths:
            ...
            if result["success"] and result.get("media_id"):
                ...
                # 替换 body 中的本地路径为 Twitter URL
                # (用于 draft 保存时保留可见引用)
                twitter_url = result.get("url", "")
                body = body.replace(path, twitter_url)
        ...
        for i, tweet_text in enumerate(tweets):
            ...
            try:
                ...
                if response.data and response.data.get("id"):
                    ...
                    break
            except tweepy.TweepyForbidden as e:
