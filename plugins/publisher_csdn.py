...
CSDN 无公开 API，用 Playwright 模拟发布
⚠️ 平台改前端可能失效，维护成本较高
"""
...
class CSDNPublisher(Publisher):
    ...
    config_fields = [
        {"key": "cookie", "label": "Cookie", "type": "password", "required": True,
         "placeholder": "登录 CSDN 后从浏览器 F12 复制"},
    ]
    ...
    def publish(self, article: Article, **kwargs) -> dict:
        ...
        return {
            "success": False,
            "error": "CSDN 发布需 Playwright 自动化，请配置后重试。"
    ...
    def publish_with_playwright(self, article: Article) -> dict:
        ...
        try:
            with sync_playwright() as p:
                ...
                page.goto("https://mp.csdn.net/mp_blog/creation/editor", timeout=30000)
                page.wait_for_timeout(3000)

                # 填标题
                title_input = page.locator("#article-title")
                title_input.fill(article.title)
                # 填正文（切换 Markdown 模式）
                md_btn = page.locator("text=Markdown")
                if md_btn.is_visible():
                    md_btn.click()
                page.wait_for_timeout(1000)
                content_area = page.locator("#editor-content")
            ...
            return {"success": False, "error": f"CSDN Playwright 发布失败: {e}",
                    "url": "", "id": ""}
